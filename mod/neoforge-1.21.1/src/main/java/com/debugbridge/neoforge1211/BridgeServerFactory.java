...
public class BridgeServerFactory {
    ...
    private static synchronized ClassLoader extLoader() {
        ...
        URL[] urls = java.util.Arrays.stream(cp.split(java.util.regex.Pattern.quote(System.getProperty("path.separator"))))
            .filter(s -> s.contains("java-websocket") || s.contains("Java-WebSocket"))
            .map(s -> { try { return new java.io.File(s).toURI().toURL(); } catch (Exception e) { return null; } })
            .filter(u -> u != null)
            .toArray(URL[]::new);
        if (urls.length == 0) {
    ...
    public static BridgeServer create(
            ...
            ScreenshotProvider screenshotProvider) throws Exception {
