...
/**
 * 1.19 per-frame capture for the recording pipeline. Called from the render
 * thread (mixin tail of {@code Minecraft.runTick}); the underlying screenshot
 * API in 1.19 is fully synchronous, so the {@link FrameSink} fires before
 * {@link #capture} returns.
 *
 * <p>Mirrors {@link Minecraft119ScreenshotProvider} but skips the
 * {@code mc.execute} hop (we're already on the render thread) and writes the
 * raw ARGB array to the sink instead of a JPEG temp file.
 */
public final class Minecraft119FrameCapturer implements FrameCapturer {
