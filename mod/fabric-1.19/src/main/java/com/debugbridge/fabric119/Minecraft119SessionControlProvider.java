...
/**
 * Session control for 1.19. {@code disconnect} and {@code quit} queue onto the
 * game thread and complete asynchronously — their bridge response only
 ...
 */
public class Minecraft119SessionControlProvider implements SessionControlProvider {
    ...
    public void joinServer(String address, boolean acceptResourcePacks) throws Exception {
        ...
        settleGate.runWhenSettled(
                () -> {
                    leaveWorld(mc);
                    ServerData data = new ServerData("DebugBridge", address, false);
                    ...
                    ConnectScreen.startConnecting(new TitleScreen(), mc, addr, data);
                },
