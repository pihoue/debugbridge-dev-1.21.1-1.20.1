...
JAR_NAME="debugbridge-1.21.11-2.0.0.jar"
# Cleanup list: any older names we've produced for this module.
OLD_JAR_NAMES=("debugbridge-1.21.11-1.1.0.jar" "luabridge-1.21.11-1.0.0.jar" "fabric-1.21.11-1.0.0.jar" "debugbridge-1.21.11-1.0.0.jar")
SOURCE_JAR="${MOD_DIR}/fabric-1.21.11/build/libs/${JAR_NAME}"
TARGET_JAR="${TARGET_DIR}/${JAR_NAME}"

echo "Building DebugBridge mod (fabric-1.21.11)..."
cd "${MOD_DIR}"
# Wipe stale build outputs from any previous archivesName (e.g. the old "fabric-*" or "luabridge-*" jars).
for old in "${OLD_JAR_NAMES[@]}"; do
    rm -f "${MOD_DIR}/fabric-1.21.11/build/libs/${old}"
done
JAVA_HOME=/opt/homebrew/opt/openjdk@21 ./gradlew :fabric-1.21.11:build
...
# Remove any previously deployed jar with an old name so two copies don't co-load.
for old in "${OLD_JAR_NAMES[@]}"; do
