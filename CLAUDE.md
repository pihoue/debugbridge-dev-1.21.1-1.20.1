...
- `disconnect`/`quit` are fire-and-acknowledge: the provider queues the operation on the game thread and the response returns immediately. `joinServer` instead blocks (bounded, 60 s) until the client has settled — no loading overlay for a few consecutive polls — before connecting, then acks: connecting mid-reload (a join issued right after launch, while the startup reload is in flight) made the server-resource-pack application collide with that reload and silently drop the pack (`ClientSettleGate` in core has the full story). A never-settling client surfaces as an error response. Either way, poll `snapshot` (player present = in world) and `screenInspect` (e.g. DisconnectedScreen) for the outcome.
